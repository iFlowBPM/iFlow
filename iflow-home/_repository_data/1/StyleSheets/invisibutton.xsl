	...
	<xsl:if test="type='link'">
		...
		<xsl:choose>
			...
			<xsl:choose>
				...
				<xsl:attribute name="onclick">
							<xsl:text>window.open('</xsl:text><xsl:apply-templates select="href" /><xsl:text>',
				...
				<td>
					<xsl:if test="asSignatures='true'">
						<img class="toolTipImg" id="lock_{id}" border="0" width="16" height="16" 
							src="{$url_prefix}/images/lock.png" alt="Assinado" title="Este documento jï¿½ foi assinado">
						</img>
					</xsl:if>
					<xsl:if test="asSignatures!='true'">
