...
.scrollableContainer {
	height: expression((this.scrollHeight > 429 || this.scrollHeight == 0) ? '430px' : 'auto');
}
  ...
  <xsl:template match="field">
	...
	<xsl:if test="type='selection'">
      ...
      <xsl:if test="type='arraytable'">
		...
		<div id="tableContainer" class="tableContainer scrollableContainer">
        <table class="arraytable scrollableTable scrollTable">
          <xsl:variable name="rowCount" select="count(row)" />
          <xsl:variable name="colCount" select="count(row/col)" />
		  <thead class="fixedHeader" id="fixedHeader">

		  <xsl:for-each select="row">
		    <xsl:if test="col[1]/header='true'">
			...
			<xsl:choose>
              ...
              <xsl:for-each select="col[header='true']">
                <th>
