...
public class BlockP17040ValidateCCIN extends BlockP17040Validate {
	...
	public ArrayList<ValidationError> validate(UserInfoInterface userInfo, ProcessData procData, DataSource datasource,
		...
		for (Integer infInstId : infInstIdList) {
			HashMap<String, Object> infInstValues = fillAtributtes(null, datasource, userInfo,
			...
			//projFinan
			if(!isValidDomainValue(userInfo, datasource, "T_EFP","" + infInstValues.get("projFinan")))
				result.add(new ValidationError("CI007", "infInst", "projFinan", infInstId));
			
			//litigJud
			if(!isValidDomainValue(userInfo, datasource, "T_LJU","" + infInstValues.get("litigJud")))
				result.add(new ValidationError("CI007", "infInst", "litigJud", infInstId));
			
			//paisLegis
			if(!isValidDomainValue(userInfo, datasource, "T_TER","" + infInstValues.get("paisLegis")))
				result.add(new ValidationError("CI013", "infInst", "paisLegis", infInstId));
			
			//canalComer
			if(!isValidDomainValue(userInfo, datasource, "T_CCO","" + infInstValues.get("canalComer")))
				result.add(new ValidationError("CI104", "infInst", "canalComer", infInstId));
			
			//clausRenun
			if(!isValidDomainValue(userInfo, datasource, "T_REN","" + infInstValues.get("clausRenun")))
				result.add(new ValidationError("CI015", "infInst", "clausRenun", infInstId));
			
			//subvProtocolo
			if(!isValidDomainValue(userInfo, datasource, "T_SPR","" + infInstValues.get("subvProtocolo")))
			...
			//tpInst
			if(!isValidDomainValue(userInfo, datasource, "T_TIN", (String)infInstValues.get("tpInst")))
				result.add(new ValidationError("CI031", "infInst", "tpInst", infInstId));
			
			//moeda
			if(!isValidDomainValue(userInfo, datasource, "T_DIV", (String)infInstValues.get("moeda")))
			...
			//dirReemblme
			if(!isValidDomainValue(userInfo, datasource, "T_REB", (String)infInstValues.get("dirReemblme")))
				result.add(new ValidationError("CI044", "infInst", "dirReemblme", infInstId));
			
			//recurso
			if(!isValidDomainValue(userInfo, datasource, "T_RCU", (String)infInstValues.get("recurso")))
				result.add(new ValidationError("CI046", "infInst", "recurso", infInstId));
			
			//tpTxJuro
			if(!isValidDomainValue(userInfo, datasource, "T_TTJ", (String)infInstValues.get("tpTxJuro")))
				result.add(new ValidationError("CI047", "infInst", "tpTxJuro", infInstId));
			
			//freqAtualizTx
			if(!isValidDomainValue(userInfo, datasource, "T_FTJ", (String)infInstValues.get("freqAtualizTx")))
				result.add(new ValidationError("CI050", "infInst", "freqAtualizTx", infInstId));
			
			//txRef
			if(!isValidDomainValue(userInfo, datasource, "T_TXR", (String)infInstValues.get("txRef")))
			...
			//finalidade
			if(!isValidDomainValue(userInfo, datasource, "T_FIN", (String)infInstValues.get("finalidade")))
			...
			//tpAmort
			if(!isValidDomainValue(userInfo, datasource, "T_TAM", (String)infInstValues.get("tpAmort")))
				result.add(new ValidationError("CI063", "infInst", "tpAmort", infInstId));
			
			//freqPagam
			if(!isValidDomainValue(userInfo, datasource, "T_FPG", (String)infInstValues.get("freqPagam")))
				result.add(new ValidationError("CI065", "infInst", "freqPagam", infInstId));
			
			//divSubor
			if(!isValidDomainValue(userInfo, datasource, "T_DIS", (String)infInstValues.get("divSubor")))
				result.add(new ValidationError("CI067", "infInst", "divSubor", infInstId));
			
			//instFiduc
			if(!isValidDomainValue(userInfo, datasource, "T_FID", (String)infInstValues.get("instFiduc")))
			...
			//tpNeg
			if(!isValidDomainValue(userInfo, datasource, "T_TNE", (String)infInstValues.get("tpNeg")))
			...
			//tpTitulariz
			if(!isValidDomainValue(userInfo, datasource, "T_TTI", (String)infInstValues.get("tpTitulariz")))
				result.add(new ValidationError("CI076", "infInst", "tpTitulariz", infInstId));
			
			//seguros
			if(!isValidDomainValue(userInfo, datasource, "T_SAS", (String)infInstValues.get("seguros")))
			...
			for (Integer infRiscoInstId : infRiscoInstIdList) {
				HashMap<String, Object> infRiscoInstValues = fillAtributtes(null, datasource, userInfo, "select * from infRiscoInst where id = {0} ",
				...
				//tpAvalRiscoInst
				if(!isValidDomainValue(userInfo, datasource, "T_TAR", (String)infRiscoInstValues.get("tpAvalRiscoInst")))
					result.add(new ValidationError("CI079", "infRiscoInst", "tpAvalRiscoInst", infRiscoInstId));
				
				//sistAvalRiscoInst
				if(!isValidDomainValue(userInfo, datasource, "T_SAR", (String)infRiscoInstValues.get("sistAvalRiscoInst")))
				...
				//tipoPDInst
				if(!isValidDomainValue(userInfo, datasource, "T_TPD", (String)infRiscoInstValues.get("tipoPDInst")))
			...
			for (Integer ligInstId : ligInstIdList) {
				HashMap<String, Object> ligInstValues = fillAtributtes(null, datasource, userInfo,
						"select * from ligInst where id = {0} ", new Object[] { ligInstId });
				
				//tpLigInst
				if(!isValidDomainValue(userInfo, datasource, "T_LIN", (String)ligInstValues.get("tpLigInst")))
			...
			for (Integer entSindId : lstEntSindIdList) {
				HashMap<String, Object> entSindValues = fillAtributtes(null, datasource, userInfo,
						"select * from entSind where id = {0} ", new Object[] { entSindId });
				
				//relEntsind
				if(!isValidDomainValue(userInfo, datasource, "T_RPS", (String)entSindValues.get("relEntsind")))
			...
			for (Integer caractEspId : lstCaractEspIdList) {
				HashMap<String, Object> caractEspValues = fillAtributtes(null, datasource, userInfo, "select * from caractEsp where id = {0} ",
						new Object[] { caractEspId });
				
				//tpCaractEsp
				if(!isValidDomainValue(userInfo, datasource, "T_CEP", (String)caractEspValues.get("tpCaractEsp")))
