...
public class BlockP17040ValidateCCIN extends BlockP17040Validate {
	...
	public ArrayList<ValidationError> validate(UserInfoInterface userInfo, ProcessData procData, Connection connection,
		...
		for (Integer infInstId : infInstIdList) {
			...
			String tpCaractEsp = (String) infInstValues.get("tpCaractEsp");
			...
			if (StringUtils.isBlank(canalComer) && fimDe2018.getTime().before(dtIniInst)
					&& (StringUtils.equals("005", tpCaractEsp) || StringUtils.equals("006", tpCaractEsp)))
			...
			if (StringUtils.equals("005", tpCaractEsp)
			...
			//dtIniCarJur
			if(dtIniCarJur==null)
				result.add(new ValidationError("INC038", "infInst", "dtIniCarJur", infInstId));
			...
			if (!isValidDomainValue(userInfo, connection, "T_REB", (String) infInstValues.get("dirReemblme")))
				result.add(new ValidationError("CI044", "infInst", "dirReemblme", infInstId, (String) infInstValues.get("dirReemblme")));
			...
			if (StringUtils.isBlank(tpTxJuro) && StringUtils.equals("005", tpCaractEsp))
				result.add(new ValidationError("CI048", "infInst", "tpTxJuro", infInstId));
			if (StringUtils.isBlank(tpTxJuro) && StringUtils.equals("002", tpCaractEsp))
			...
			if ((StringUtils.equals(tpTxJuro,"002") && !StringUtils.equals(txRef, "0000")) || (!StringUtils.equals(tpTxJuro,"002") && StringUtils.equals(txRef, "0000")))
				result.add(new ValidationError("CI095", "infInst", "txRef", infInstId, txRef));
			if(StringUtils.isBlank(txRef) && StringUtils.equals(tpCaractEsp, "002"))
				result.add(new ValidationError("CI108", "infInst", "txRef", infInstId, txRef));
			
			//TAEG
			BigDecimal TAEG = (BigDecimal) infInstValues.get("TAEG");
			if (TAEG==null && StringUtils.equals(tpCaractEsp, "005"))
			...
			if (TAE==null && StringUtils.equals(tpCaractEsp, "006") &&  dtIniInst.before(inicioDe2018.getTime()))
				result.add(new ValidationError("CI097", "infInst", "TAE", infInstId, TAE));
			
			//spread
			BigDecimal spread = (BigDecimal) infInstValues.get("spread");
			if(spread==null && StringUtils.equals(tpCaractEsp, "006"))
			...
			if (StringUtils.isBlank(tpAmort) && StringUtils.equals(tpCaractEsp, "006"))
			...
			if (!isValidDomainValue(userInfo, connection, "T_FPG", (String) infInstValues.get("freqPagam")))
				result.add(new ValidationError("CI065", "infInst", "freqPagam", infInstId, (String) infInstValues.get("freqPagam")));

			// divSubor
			if (!isValidDomainValue(userInfo, connection, "T_DIS", (String) infInstValues.get("divSubor")))
				result.add(new ValidationError("CI067", "infInst", "divSubor", infInstId, (String) infInstValues.get("divSubor")));

			// instFiduc
			if (!isValidDomainValue(userInfo, connection, "T_FID", (String) infInstValues.get("instFiduc")))
				result.add(new ValidationError("CI068", "infInst", "instFiduc", infInstId, (String) infInstValues.get("instFiduc")));
			...
			if(varFV==null && retrieveSimpleField(connection, userInfo,
					"select from ligInst where tpLigInst='003' and infInst_id = {0} ",
					new Object[] { infInstId }).size()>0)
			...
			if (percDifCap != null && (percDifCap.compareTo(BigDecimal.ZERO)==-1 || percDifCap.compareTo(new BigDecimal(100))==1))
				result.add(new ValidationError("CI074", "infInst", "percDifCap", infInstId, percDifCap));
			if(percDifCap==null && StringUtils.equals(tpCaractEsp, "006"))
			...
			if(StringUtils.isBlank(seguros) && StringUtils.equals(tpCaractEsp, "005"))
			...
			for (Integer infRiscoInstId : infRiscoInstIdList) {
				...
				// tpAvalRiscoInst
				if (!isValidDomainValue(userInfo, connection, "T_TAR",
						(String) infRiscoInstValues.get("tpAvalRiscoInst")))
					result.add(new ValidationError("CI079", "infRiscoInst", "tpAvalRiscoInst", infRiscoInstId));

				// sistAvalRiscoInst
				if (!isValidDomainValue(userInfo, connection, "T_SAR",
						(String) infRiscoInstValues.get("sistAvalRiscoInst")))
