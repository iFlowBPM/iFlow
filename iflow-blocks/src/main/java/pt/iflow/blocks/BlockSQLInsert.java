	...
	public Port after(UserInfoInterface userInfo, ProcessData procData) {
		...
		try {
			...
			Logger.warning(login, this, "after",
					"Unable to parse DataSource during insert procedure (received: " + sDataSource + ").", e);
		}
		...
		if (sQuery == null) {
			...
			try {
				...
				sInto = null;
				Logger.warning(login, this, "after",
						"Unable to parse table name during insert procedure (received: " + sInto + ").", e);
			}
			try {
				...
				sNames = null;
				Logger.warning(login, this, "after",
						"Unable to parse column names during insert procedure (received: " + sNames + ").", e);
			}
			try {
				...
				sValues = null;
				Logger.warning(login, this, "after",
						"Unable to parse column values during insert procedure (received: " + sValues + ").", e);
			}
			...
			if (sInto == null || sNames == null || sValues == null) {
				...
				if (this.isSystemTable(sDataSource, sInto)) {
					Logger.error(login, this, "after",
							procData.getSignature() + "Into '" + sInto + "' is a system table");
					outPort = portError;
				} else {
