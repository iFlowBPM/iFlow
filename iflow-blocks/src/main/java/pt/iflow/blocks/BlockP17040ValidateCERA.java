		...
		for (Integer comRiscoEnt_id : comRiscoEntIdList) {
			...
			for (Integer riscoEnt_id : riscoEntIdList) {
				...
				if(retrieveSimpleField(connection, userInfo,
					...
					result.add(new ValidationError("EF013", "riscoEnt", "idEnt_id", idEntValue, riscoEnt_id, idEnt_id));;
				...
				for (Integer clienteRel_id : clienteRelIdList) {
					...
					if(!isValidDomainValue(userInfo, connection, "T_TRL",motivoRel))
						result.add(new ValidationError("RE013", "clienteRel", "motivoRel", idEntValue, clienteRel_id, motivoRel));;
				}
				...
				for (Integer infRiscoEnt_id : infRiscoEntIdList) {
					...
					if(!isValidDomainValue(userInfo, connection, "T_DST",estadoInc))
						result.add(new ValidationError("RE004", "infRiscoEnt", "estadoInc", idEntValue, infRiscoEnt_id, estadoInc));
					
					//dtAltEstadoInc
					Date dtAltEstadoInc = (Date) infRiscoEntValues.get("dtAltEstadoInc");
					if(dtAltEstadoInc==null && StringUtils.isNotBlank(estadoInc))
						result.add(new ValidationError("RE005", "infRiscoEnt", "dtAltEstadoInc", idEntValue, infRiscoEnt_id));
					if(dtAltEstadoInc!=null && dtAltEstadoInc.after(new Date()))
						result.add(new ValidationError("RE006", "infRiscoEnt", "dtAltEstadoInc", idEntValue, infRiscoEnt_id, dtAltEstadoInc));
					...
					if(!isValidDomainValue(userInfo, connection, "T_RCC",entAcompanhada))
						result.add(new ValidationError("RE024", "infRiscoEnt", "entAcompanhada", idEntValue, infRiscoEnt_id, entAcompanhada));
					
					//dtApurTxEsf
					Date dtApurTxEsf = (Date) infRiscoEntValues.get("dtApurTxEsf");
					if(dtApurTxEsf!=null && dtApurTxEsf.after(new Date()))
						result.add(new ValidationError("RE018", "infRiscoEnt", "dtApurTxEsf", idEntValue, infRiscoEnt_id, dtApurTxEsf));
					...
					if(!isValidDomainValue(userInfo, connection, "T_ATE",tpAtualizTxEsf))
						result.add(new ValidationError("RE024", "infRiscoEnt", "tpAtualizTxEsf", idEntValue, infRiscoEnt_id, tpAtualizTxEsf));
					...
					for (Integer avalRiscoEnt_id : avalRiscoEntIdList) {
						...
						if(PD!=null && (PD.doubleValue()<0 || PD.doubleValue()>100))
							result.add(new ValidationError("RE007", "avalRiscoEnt", "PD", idEntValue, avalRiscoEnt_id, PD));
						
						//dtDemoFin
						Date dtDemoFin = (Date) avalRiscoEntValues.get("dtDemoFin");
						if(dtDemoFin!=null && dtDemoFin.after(new Date()))
							result.add(new ValidationError("RE017", "avalRiscoEnt", "dtDemoFin", idEntValue, avalRiscoEnt_id, dtDemoFin));
						
						//tpAvalRisco
						String tpAvalRisco = (String) avalRiscoEntValues.get("tpAvalRisco");
						if(!isValidDomainValue(userInfo, connection, "T_TAR",tpAvalRisco))
							result.add(new ValidationError("RE009", "avalRiscoEnt", "tpAvalRisco", idEntValue, avalRiscoEnt_id, tpAvalRisco));
						...
						if(!isValidDomainValue(userInfo, connection, "T_SAR",sistAvalRisco))
							result.add(new ValidationError("RE010", "avalRiscoEnt", "sistAvalRisco", idEntValue, avalRiscoEnt_id, sistAvalRisco));
						
						//dtAvalRisco
						Date dtAvalRisco = (Date) avalRiscoEntValues.get("dtAvalRisco");
						if(dtAvalRisco!=null && dtAvalRisco.after(new Date()))
							result.add(new ValidationError("RE011", "avalRiscoEnt", "dtAvalRisco", idEntValue, avalRiscoEnt_id, dtAvalRisco));
						
						//tipoPD
						String tipoPD = (String) avalRiscoEntValues.get("tipoPD");
						if(!isValidDomainValue(userInfo, connection, "T_TPD",tipoPD))
							result.add(new ValidationError("RE023", "avalRiscoEnt", "tipoPD", idEntValue, avalRiscoEnt_id, tipoPD));
						if(StringUtils.isBlank(tipoPD) && PD!=null)
							result.add(new ValidationError("RE026", "avalRiscoEnt", "tipoPD", idEntValue, avalRiscoEnt_id));
					}
				}
