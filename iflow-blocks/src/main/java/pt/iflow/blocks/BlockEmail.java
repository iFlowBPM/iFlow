...
public class BlockEmail extends Block {
	...
	public Port after(UserInfoInterface userInfo, ProcessData procData) {
		...
		if (!Const.bUSE_EMAIL) {
			outPort = portError;
			Logger.warning(user, this, "after", procData.getSignature()
			...
			try {
				...
				try {
					...
					Logger.warning(user, this, "after", procData.getSignature() + "exception transforming to: " + aTo);
				}
				String from = aFrom;
				try {
				...
				try {
					...
					for (int i = 0; i < docsVar.size(); i++) {
						...
						if (docsVar.getItem(i).getValue() instanceof Integer)
							doc = docBean.getDocument(userInfo, procData,
									((Integer) docsVar.getItem(i).getValue()).intValue());
						else
							doc = docBean.getDocument(userInfo, procData,
									((Long) docsVar.getItem(i).getValue()).intValue());
				...
				if (tos == null || tos.isEmpty()) {
					...
					for (String mailto : tos) {
						try {
							...
							Logger.error(user, this, "after",
									procData.getSignature() + "Invalid to address: " + mailto);
						}
					}
					if (errorAddresses) {
						outPort = portError;
					...
					if (!hasTemplate(template)) {
						// if no template defined, subject, message and from
						// have to be defined
					...
					if (!outPort.equals(portError)) {
						...
						if (hasTemplate(template)) {
							...
							if (etemp != null) {
								...
								// default variables
								if (from != null)
									htProps.put("from", from);
								if (subject != null)
									htProps.put("subject", subject);
								if (message != null)
									htProps.put("message", message);
						...
						if (email != null) {
							for (String mailto : tos) {
								...
								if (email.sendMsg()) {
									...
									Logger.info(user, this, "after",
											procData.getSignature() + "email sent to " + mailto);
								} else {
									Logger.error(user, this, "after",
											procData.getSignature() + "email NOT sent to " + mailto);
								}
							}
							outPort = portSuccess;
						} else {
				...
				e.printStackTrace();
				Logger.error(user, this, "after", procData.getSignature() + "caught exception: " + e.getMessage(), e);
				outPort = portError;
			}
		}
	...
	private boolean hasTemplate(String template) {
		if (template == null || template.equals("") || template.equals(_sSELECT) || !template.endsWith(".et")) {
			return false;
		}
		return true;
	}
