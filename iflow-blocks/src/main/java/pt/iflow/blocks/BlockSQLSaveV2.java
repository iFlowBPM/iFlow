...
public class BlockSQLSaveV2 extends BlockSQL {

  private static final String sWIZARD = "wizard";
  private static final String sWIZARD_SEPARATOR = "@";

  public BlockSQLSaveV2(int anFlowId, int id, int subflowblockid, String filename) {
  ...
  public Port after(UserInfoInterface userInfo, ProcessData procData) {
	  ...
	  // get id of template block  
	  String templateId = null;
	    try {
	    	templateId = this.getAttribute("sqlTemplate");	      
	    ...
	    if (StringUtils.isBlank(templateId)) {
	        Logger.error(login, this, "after", procData.getSignature() + "Empty template");
	        outPort = portError;	        
	      }   else { 
	 
	    	  
	//get all blocks of the flow  
	IFlowData fd=BeanFactory.getFlowHolderBean().getFlow(userInfo, this.getFlowId());
	Vector<Block> blocks=fd.getFlow();	
	Block block=null;
	HashMap<String, String> attrs=null;
	for(int i = 0; i< blocks.size(); i++) {
		block = blocks.get(i);
		//get id of the block
		String blockId= Integer.toString(block.getId());
		if(block instanceof BlockSQLTemplate) {			
			if(blockId.equals(templateId)) {				
				break;//break FOR
			}				
		}						
	}
	...
	if(block!=null) {	
	    ...
	    //parse attributes
	    try {
	        sDataSource = attrs.get(BlockSQL.sDATASOURCE);
	      ...
	      try {
	        sTable = attrs.get(BlockSQL.sTABLE);
