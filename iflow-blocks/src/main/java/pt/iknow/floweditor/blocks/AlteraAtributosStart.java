...
public class AlteraAtributosStart extends AbstractAlteraAtributos implements AlteraAtributosInterface {
	...
	public AlteraAtributosStart(FlowEditorAdapter adapter) {
		...
		columnNamesCatalog = new String[] { adapter.getString("AlteraAtributosCatalogVars.vars.columnlabel"), //$NON-NLS-1$
		...
		columnNamesProps = new String[] { adapter.getString("AlteraAtributosStart.property.column.name"), //$NON-NLS-1$
				adapter.getString("AlteraAtributosStart.description.column.name"), //$NON-NLS-1$
		};

		columnNamesVars = new String[] { adapter.getString("AlteraAtributosStart.ws.vars.tab.label"), //$NON-NLS-1$
		};

		columnNamesMailStart = new String[] { adapter.getString("AlteraAtributosStart.mailstart.mapping.prop"), //$NON-NLS-1$
	...
	public String[][] getNewAttributes() {
		...
		for (int i = 0; i < dataMSVars.length; i++) {
			...
			if (StringUtilities.isNotEmpty(msprop) && StringUtilities.isNotEmpty(msvar)
		...
		for (int i = 0, k = 0; i < dataMSVars.length; i++) {
			...
			if (StringUtilities.isNotEmpty(msprop) && StringUtilities.isNotEmpty(msvar)
				...
				newAttributes[beginIndex + k][0] = AlteraAtributosStart.sMAIL_START_VARS_PREFIX
						+ AlteraAtributosStart.MAILSTART_MAIL_PROP + i;
				newAttributes[beginIndex + k][1] = msprop;
				newAttributes[beginIndex + k][2] = AlteraAtributosStart.MAILSTART_MAIL_PROP;
				k++;
				newAttributes[beginIndex + k][0] = AlteraAtributosStart.sMAIL_START_VARS_PREFIX
						+ AlteraAtributosStart.MAILSTART_FLOW_VAR + i;
	...
	private List<Object[]> getDynamicVariables() {
		...
		for (String var : DynamicBindDelegate.getDynamicVariables()) {
			retObj.add(new Object[] { var, null, DynamicBindDelegate.getDynamicVariableDataType(var), Boolean.FALSE,
					adapter.getString("DynamicBindDelegate.catalog." + var + ".publicname"), null });
		}
		return retObj;
	}
	...
	public void setDataIn(String title, List<Atributo> atributos) {
		...
		if (desenho != null) {
			...
			for (Atributo attr : catalogo) {
				if (StringUtilities.isEmpty(attr.getNome()))
					continue;
				String sType = attr.getDataType();
				DataTypeEnum type = DataTypeEnum.getDataType(sType);
				Object[] elem = new Object[] { attr.getNome(), attr.getInitValue(), type, attr.isSearchable(),
						attr.getPublicName(), attr.getFormat(), };
				dataAux.add(elem);
			...
			for (int row = 0; row < reservedDataSize; row++) {
				for (int col = 0; col < columnNamesCatalog.length; col++) {
					...
					if (col == 3) {
						...
						// catalogModel.setCellEditable(row, col, true); TODO
						// Implementar pesquisavel nas variáveis Bindable
					} else {
	...
	private void jbInit() {
...

		{
...

			{
				...
				group.add(noDetail);
				// DEFAULT DETAIL
				JRadioButton defaultDetail = new JRadioButton(
						adapter.getString("AlteraAtributosStart.detail.radio.default"));
				...
				jButtonDetail.setEnabled(false);
				// FORM DETAIL-BLOCKID
				JRadioButton existFormDetail = new JRadioButton(
						adapter.getString("AlteraAtributosStart.detail.radio.existingform"));
	...
	private boolean validateData() {
		...
		if (!dataIsValid) {
			...
			boolean detailOk = !(detailType == sDETAIL_FORM_EXISTINGBLOCKFORM
					&& (StringUtils.isEmpty(detailFormBID) || !StringUtils.isNumeric(detailFormBID)));
