...
public class FlowData implements IFlowData, Serializable {
	...
	protected FlowData(final UserInfoInterface userInfo, final int anFlowId, final String asName,
			final String asFileName, final long created, final long lastModified, final int seriesId, String typeCode,
		...
		try {
			...
			if (xmlcv != null && xmlcv.getXmlCatalogVarAttribute().size() > 0) {
				for (int i = 0; i < xmlcv.getXmlCatalogVarAttribute().size(); i++) {
					...
					this.setCatalogueVar(attr.getName(), attr.getInitVal(), dataType, attr.isIsSearchable(),
							attr.getPublicName(), format);
	...
	private InstantiationResult instantiateFlow(UserInfoInterface ui, XmlFlow aXmlFlow, int anFlowId,
		...
		for (int b = 0; b < aXmlFlow.getXmlBlock().size(); b++) {
			...
			Integer blockId = new Integer(block.getId().intValue() + offset);
			...
			try {
				...
				for (int portNumber = 0; portNumber < block.getXmlPort().size(); portNumber++) {
					...
					port.setConnectedBlockId(xmlPort.getConnectedBlockId().intValue() + offset);
				...
				throw new Exception("Erro ao criar porto " + xmlPort.getName() + " para o bloco " + block.getName(), e);
			}
			...
			bBlock.setBlockRunningInPopup(block.getPopupReturnBlockId().intValue());
	...
	@Deprecated
	private InstantiationResult instantiateSubFlow(UserInfoInterface ui, XmlBlockType xmlblock, int anFlowId,
			List<FlowSetting> alSettings, Hashtable<Integer, Block> htBlocks, int offset) throws Exception {
		...
		try {
			...
			port = new Port();
			port.setName("portIn");
			port.setConnectedBlockId(xmlPort.getConnectedBlockId().intValue() + (offset - iOFFSET));
			port.setConnectedPortName(xmlPort.getConnectedPortName());
	...
	private void setCatalogueVar(String sVar, String sValue, DataTypeEnum sType, boolean isSearchable,
			String sPublicName, String sDefaultFormat) {
	...
	private void buildForkJoinDepPathImpl(UserInfoInterface userInfo, Hashtable<Integer, Block> htBlocks,
		...
		if (block.getClass().getName().indexOf("BlockSincronizacao") != -1
			...
			for (int i = 0; outports != null && i < outports.length; i++) {
				...
				nextBlockId = new Integer(outports[i].getConnectedBlockId());
				nextBlock = htBlocks.get(nextBlockId);
				this.buildForkJoinDepPathImpl(userInfo, htBlocks, new HashSet<Integer>(alStates), nextBlock, lastFJD);
			}
		}
	}
	...
	private boolean searchDeep(UserInfoInterface userInfo, Block currentBlock, HashSet<Integer> processedBlocks) {
			...
			if (Logger.isDebugEnabled()) {
				Logger.debug(userInfo.getUtilizador(), this, "searchDeep(UserInfoInterface,Block)",
						"end : block id=" + currentBlock.getId() + ". Found "
								+ (currentBlock.isForwardBlock() ? "forward" : "end") + " block, returning 'true'.");
			}
			return true;
		...
		if (Logger.isDebugEnabled()) {
			Logger.debug(userInfo.getUtilizador(), this, "searchDeep(UserInfoInterface,Block)",
					"end : block id=" + currentBlock.getId() + ". Finished deep search algorithm, returning 'true'.");
		}
		return true;
	}
	...
	public FlowDataConvert toFlowDataConvert() {
		...
		// _vFlow has blocks, AXIS has problems with custom type Block, perphaps all
		// blocks types needs mapping in wsdd
		...
		// _detailForm has blocks, AXIS has problems with custom type Block, perphaps
		// all blocks types needs mapping in wsdd
	...
	static class ForbiddenBlockException extends SecurityException {
		...
		ForbiddenBlockException(String block) {
			super();
			this.msg = MessageFormat.format("A licença não permite blocos do tipo {0}.", new Object[] { block });
			;
		}
