...
<%
	...
	if ("del".equals(sOper)) {
		try {
			...
			if (!manager.removeOrganization(ui, orgid))
				throw new Exception();
		...
		try {
			String orgid = fdFormData.getParameter("orgid");
			UserManager manager = BeanFactory.getUserManagerBean();
			if (!manager.lockOrganization(ui, orgid))
				throw new Exception();
		} catch (Exception e) {
		...
		try {
			String orgid = fdFormData.getParameter("orgid");
			UserManager manager = BeanFactory.getUserManagerBean();
			if (!manager.unlockOrganization(ui, orgid))
				throw new Exception();
		} catch (Exception e) {
...
<div class="table_inc">
	<table class="item_list table">
		<tr class="tab_header">
			<td />
			<td><%=messages.getString("organizationadm.field.name")%></td>
			<td><%=messages.getString("organizationadm.field.description")%>
			</td>
			<td />
			<td />
		</tr>
		...
		<tr class="<%=i % 2 == 0 ? "tab_row_even" : "tab_row_odd"%>">
			<td class="itemlist_icon"><a
				href="javascript:tabber_right(4, '<%=response.encodeURL("Admin/UserManagement/organizationform.jsp")%>','orgid=<%=org.getOrganizationId()%>');"><img
					class="toolTipImg" src="images/icon_modify.png" width="16"
					height="16" border="0"
					title="<%=messages.getString("organizationadm.tooltip.edit")%>"></a>
			</td>
			<td><%=org.getName()%></td>
			<td><%=org.getDescription()%></td>
			<td class="itemlist_icon">
				<%
					if (org.isLocked()) {
				%> <a
				href="javascript:tabber_right(4, '<%=response.encodeURL("Admin/UserManagement/organizationadm.jsp")%>','oper=unlock&orgid=<%=org.getOrganizationId()%>');"><img
					class="toolTipImg" src="images/icon_offline.png" border="0"
					title="<%=messages.getString("organizationadm.tooltip.unlock")%>"></a>
				<%
					} else {
				%> <a
				href="javascript:tabber_right(4, '<%=response.encodeURL("Admin/UserManagement/organizationadm.jsp")%>','oper=lock&orgid=<%=org.getOrganizationId()%>');"><img
					class="toolTipImg" src="images/icon_online.png" border="0"
					title="<%=messages.getString("organizationadm.tooltip.lock")%>"></a>
				<%
					}
				%>
			</td>
			<td class="itemlist_icon"><a
				href="javascript:if (confirm('<%=messages.getString("organizationadm.confirm.delete")%>') ) tabber_right(4, '<%=response.encodeURL("Admin/UserManagement/organizationadm.jsp")%>','oper=del&orgid=<%=org.getOrganizationId()%>');"><img
					class="toolTipImg" src="images/icon_delete.png" border="0"
					title="<%=messages.getString("organizationadm.tooltip.delete")%>"></a>
			</td>
		</tr>
		<%
			}
...
<div class="button_box centrarBotoes">
	<input class="regular_button_01 btn btn-default" type="button"
		...
		onClick="javascript:tabber_right(4, '<%=response.encodeURL("register")%>','full=false');" />
</div>
</div>
