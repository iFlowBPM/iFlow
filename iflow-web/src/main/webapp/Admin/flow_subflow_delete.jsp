...
<%
  String title = messages.getString("subflow_delete.title");
  String sPage = "Admin/flow_subflow_delete";
  ...
  if (StringUtils.isEmpty(sFlowFile) && op == 1) {
    op = 0;
    sbError.append(messages.getString("subflow_delete.error.invalid"));
  }
  ...
  if (op == 1 && !procsChecked) {
    sbError.append(messages.getString("subflow_delete.confirm")).append("<br>");
    op = 0;
    ...
    if (flow != null) {
      ...
      try {
    	...
    	if (undeploySuccess != null) {
      		sbError.append(messages.getString("subflow_delete.error.net_deleted")).append("<br>");
      		op = 0;    		
    	}
    	else {
        	boolean result = BeanFactory.getFlowHolderBean().deleteSubFlow(userInfo, sFlowFile, true);
        	if (result) {
          		sHtml = messages.getString("subflow_delete.sucess");
        	} else {
          		sbError.append(messages.getString("subflow_delete.error.net_deleted")).append("<br>");
          		op = 0;
        	}
        ...
        e.printStackTrace();
        sbError.append(messages.getString("subflow_delete.error.internal")).append("<br>");
      }

    }
  } else {
    flowid = Integer.parseInt(sFlowId);
    sFlowFile = BeanFactory.getFlowHolderBean().getSubFlowFileName(userInfo, flowid);
    sHtml = messages.getString("subflow_delete.confirm");
  }
%>
...
<ol>
  <if:formInput name="dummy2" labelkey="subflow_delete.flowfile" type="text" value='<%=sFlowFile%>' edit="false" />
  <if:formInput name="dummy3" labelkey="subflow_delete.flowname" type="text" value='<%=sFlowName%>' edit="false" />
  <if:formInput name="procs"  labelkey="subflow_delete.agree" type="checkbox" value="false" edit="true" />
</ol>
</fieldset>
<% } %>
...
%>
  <input class="regular_button_01 btn btn-default" type="button" name="back" value="<%=messages.getString("button.back")%>" 
         onClick="javascript:tabber_right(4, '<%=response.encodeURL("Admin/flow_subflow.jsp") %>','ts=<%=ts%>');"/>
<%
  } else {
%>
  ...
  <input class="regular_button_01 btn btn-default" type="button" name="delete" value="<%=messages.getString("button.delete")%>" 
         onClick="javascript:tabber_right(4, '<%=response.encodeURL("Admin/flow_subflow_delete.jsp") %>','op=1&' + get_params(document.flowdelete));"/>
<%
  }
%>
</fieldset>
