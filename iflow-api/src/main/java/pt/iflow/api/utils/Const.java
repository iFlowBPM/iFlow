...
public class Const {
	...
	public static final String sFORCE_NOTIFY_FOR_PROFILES_DESC = "For&ccedil;ar Notifica&ccedil;&atilde;o para Perfis (quando"
			+ sNOTIFY_USER + "=" + sNOTIFY_USER_NO + ")";
	public static final String sDENY_NOTIFY_FOR_PROFILES = "DENY_NOTIFY_FOR_PROFILE";
	public static final String sDENY_NOTIFY_FOR_PROFILES_DESC = "Abortar Notifica&ccedil;&atilde;o para Perfis (quando "
  ...
  public static Boolean ENCRYPT_FILESYSTEM_DOCS = false;
	...
	// START constantes modulo symmetric encryption / decryption of files
	public static boolean bENCRYPT_DECRYPT_FILES = false;

	public static String sENCRYPT_DECRYPT_PROVIDER = null;
	public static String sENCRYPT_DECRYPT_PROVIDER_CLASS = null;

	public static String sENCRYPT_DECRYPT_KEYSTORE_INSTANCE = null;
	public static String sENCRYPT_DECRYPT_KEYSTORE_PATH = null;
	public static String sENCRYPT_DECRYPT_KEYSTORE_PASS = null;

	public static String sENCRYPT_DECRYPT_algorithm_CIPHER = null;
	public static String sENCRYPT_DECRYPT_algorithm_MODE = null;
	public static String sENCRYPT_DECRYPT_algorithm_PADDING = null;
	public static String sENCRYPT_DECRYPT_ALGORITHM = null;

	public static int iENCRYPT_DECRYPT_SECRETKEY_LENGTH = -1;
	public static int iENCRYPT_DECRYPT_SECRETKEY_FILE_USAGE = -1;

	public static int iENCRYPT_DECRYPT_IVLENGHT = -1;
	public static int iENCRYPT_DECRYPT_IVLENGHT_MAX = -1;
	public static int iENCRYPT_DECRYPT_IVLENGHT_MIN = -1;
	public static int iENCRYPT_DECRYPT_TAGLENGHT = -1;
	// END constantes modulo symmetric encryption / decryption of files

	/**
	 * Indicação se fica ou não na página. Vai sair daqui e morar para um sitio
	 * muito mais bonito.
	 */
	...
	public static synchronized void updateConstants() {
		...
		try {
			SEARCH_ALL_USER_PROCS_BY_DEFAULT = Boolean
					.parseBoolean(Setup.getProperty("SEARCH_ALL_USER_PROCS_BY_DEFAULT"));
		} catch (Exception e) {
		}
		...
		bMAIL_STARTTLS = StringUtils.isNotEmpty(stmp)
				&& ArrayUtils.contains(new String[] { "yes", "true", "on", "1" }, stmp.toLowerCase());
		...
		bUSE_EMAIL = StringUtils.isNotEmpty(stmp)
				&& ArrayUtils.contains(new String[] { "yes", "true", "on", "1" }, stmp.toLowerCase());

		stmp = Setup.getProperty("EMAIL_MANAGER");
		bEMAIL_MANAGER = StringUtils.isNotEmpty(stmp)
				&& ArrayUtils.contains(new String[] { "yes", "true", "on", "1" }, stmp.toLowerCase());

		stmp = Setup.getProperty("USE_SCANNER");
		bUSE_SCANNER = StringUtils.isNotEmpty(stmp)
				&& ArrayUtils.contains(new String[] { "yes", "true", "on", "1" }, stmp.toLowerCase());
		...
		if (null == AUTHENTICATION_TYPE)
			AUTHENTICATION_TYPE = AUTHENTICATION_BOTH;

		stmp = Setup.getProperty("USE_INDIVIDUAL_LOCALE");
		USE_INDIVIDUAL_LOCALE = StringUtils.isNotEmpty(stmp)
				&& ArrayUtils.contains(new String[] { "yes", "true", "on", "1" }, stmp.toLowerCase());
		stmp = Setup.getProperty("ASK_LOCALE_AT_LOGIN");
		ASK_LOCALE_AT_LOGIN = StringUtils.isNotEmpty(stmp)
				&& ArrayUtils.contains(new String[] { "yes", "true", "on", "1" }, stmp.toLowerCase());
		stmp = Setup.getProperty("HASHED_DOCUMENT_URL");
		HASHED_DOCUMENT_URL = StringUtils.isNotEmpty(stmp)
				&& ArrayUtils.contains(new String[] { "yes", "true", "on", "1" }, stmp.toLowerCase());
		...
		SEARCH_FLOW_STATE_HISTORY = StringUtils.isEmpty(stmp)
				|| ArrayUtils.contains(new String[] { "yes", "true", "sim", "1" }, stmp.toLowerCase());
		...
		DISABLE_DATASOURCE_MANAGEMENT = StringUtils.isEmpty(stmp)
				|| ArrayUtils.contains(new String[] { "yes", "true", "sim", "1" }, stmp.toLowerCase());

		stmp = Setup.getProperty("SAVE_PROCESSHISTORY_METHOD");
		if (stmp != null && (stmp.equalsIgnoreCase(SAVE_PROCESSHISTORY_METHOD_COMPRESS)
				|| stmp.equalsIgnoreCase(SAVE_PROCESSHISTORY_METHOD_NOTHING)))
		...
		// START constantes modulo symmetric encryption / decryption of files
		try {
			bENCRYPT_DECRYPT_FILES = Boolean.parseBoolean(Setup.getProperty("ENCRYPT_DECRYPT_FILES"));
		} catch (Exception e) {
			bENCRYPT_DECRYPT_FILES = false;
		}
		try {
			sENCRYPT_DECRYPT_PROVIDER = Setup.getProperty("ENCRYPT_DECRYPT_PROVIDER");
		} catch (Exception e) {
			sENCRYPT_DECRYPT_PROVIDER="BC";
		}
		try {
			sENCRYPT_DECRYPT_PROVIDER_CLASS = Setup.getProperty("ENCRYPT_DECRYPT_PROVIDER_CLASS");
		} catch (Exception e) {
			sENCRYPT_DECRYPT_PROVIDER_CLASS="rg.bouncycastle.jce.provider.BouncyCastleProvider";
		}
		try {
			sENCRYPT_DECRYPT_KEYSTORE_INSTANCE = Setup.getProperty("ENCRYPT_DECRYPT_KEYSTORE_INSTANCE");
		} catch (Exception e) {
			sENCRYPT_DECRYPT_KEYSTORE_INSTANCE = "BKS";
		}
		try {
			sENCRYPT_DECRYPT_KEYSTORE_PATH = Setup.getProperty("ENCRYPT_DECRYPT_KEYSTORE_PATH");
		} catch (Exception e) {
			sENCRYPT_DECRYPT_KEYSTORE_PATH = "C:/Users/dmarques/testbks.bks";
		}
		try {
			sENCRYPT_DECRYPT_KEYSTORE_PASS = Setup.getProperty("ENCRYPT_DECRYPT_KEYSTORE_PASS");
		} catch (Exception e) {
			sENCRYPT_DECRYPT_KEYSTORE_PASS = "arroz";
		}
		try {
			sENCRYPT_DECRYPT_algorithm_CIPHER = Setup.getProperty("ENCRYPT_DECRYPT_algorithm_CIPHER");
		} catch (Exception e) {
			sENCRYPT_DECRYPT_algorithm_CIPHER = "AES";
		}
		try {
			sENCRYPT_DECRYPT_algorithm_MODE = Setup.getProperty("ENCRYPT_DECRYPT_algorithm_MODE");
		} catch (Exception e) {
			sENCRYPT_DECRYPT_algorithm_MODE = "GCM";
		}
		try {
			sENCRYPT_DECRYPT_algorithm_PADDING = Setup.getProperty("ENCRYPT_DECRYPT_algorithm_PADDING");
		} catch (Exception e) {
			sENCRYPT_DECRYPT_algorithm_PADDING = "NoPadding";
		}
		sENCRYPT_DECRYPT_ALGORITHM = sENCRYPT_DECRYPT_algorithm_CIPHER +"/"+ sENCRYPT_DECRYPT_algorithm_MODE +"/"+ sENCRYPT_DECRYPT_algorithm_PADDING;
		try {
			iENCRYPT_DECRYPT_SECRETKEY_LENGTH = Setup.getPropertyInt("ENCRYPT_DECRYPT_SECRETKEY_LENGTH");
		} catch (Exception e) {
			iENCRYPT_DECRYPT_SECRETKEY_LENGTH = 128;
		}
		try {
			iENCRYPT_DECRYPT_SECRETKEY_FILE_USAGE = Setup.getPropertyInt("ENCRYPT_DECRYPT_SECRETKEY_FILE_USAGE");
		} catch (Exception e) {
			iENCRYPT_DECRYPT_SECRETKEY_FILE_USAGE=1000;
		}
		try {
			iENCRYPT_DECRYPT_IVLENGHT = Setup.getPropertyInt("ENCRYPT_DECRYPT_IVLENGHT");
		} catch (Exception e) {
			iENCRYPT_DECRYPT_IVLENGHT=12;
		}
		try {
			iENCRYPT_DECRYPT_IVLENGHT_MAX = Setup.getPropertyInt("ENCRYPT_DECRYPT_IVLENGHT_MAX");
		} catch (Exception e) {
			iENCRYPT_DECRYPT_IVLENGHT_MAX=16;
		}
		try {
			iENCRYPT_DECRYPT_IVLENGHT_MIN = Setup.getPropertyInt("ENCRYPT_DECRYPT_IVLENGHT_MIN");
		} catch (Exception e) {
			iENCRYPT_DECRYPT_IVLENGHT_MIN=12;
		}
		try {
